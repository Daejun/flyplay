...
**그래서 CPU 쪽 병목부터 줄였습니다.** 샌드박스 파리 한 마리의 시뮬레이션 1초(cProfile, 식초 설탕 방):

| 항목 | 최적화 전 | 최적화 뒤 |
|---|---:|---:|
| 실제 걸린 시간(프로파일러 없이) | 1378 ms (0.73×) | 1212 ms (0.83×) |
| MuJoCo 물리 | 834 ms | 825 ms |
| 걷기 제어기(500 Hz, 관측·접촉 힘 포함) | 약 480 ms | 131 ms |
| 겹눈 렌더링과 픽셀 읽기(10 Hz) | 225 ms | 224 ms |

첫 행을 뺀 나머지는 cProfile을 켠 채 잰 누적 시간이라, 더해도 첫 행(프로파일러 없이 잰 값)과 맞지 않습니다.

- 제어기는 FlyGym과 같은 계산을 같은 순서로 하되 매번 만들던 관절 객체·조회표를 한 번만 만들고, 제어기 사이의 물리
  20스텝은 `mj_step(nstep)` 한 번으로 돌립니다. FlyGym 원래 경로와 6000번 비교해 어긋남 0, 행동 지문도 같습니다.
- 남은 큰 몫은 물리(68%)와 겹눈(17%)이었습니다. 겹눈과 뷰어 영상은 **RTX가 아니라 Intel 내장 GPU**에서 그려지고
  있었습니다(`GL_RENDERER` = Intel(R) Graphics, 512×450 한 장에 10.9~16.5 ms).
- **RTX로 옮겼습니다.** Windows 그래픽 설정으로 `python.exe` 전체를 RTX에 묶는 대신, NVIDIA Optimus 드라이버가 OpenGL
  컨텍스트를 만들 때 읽는 환경 변수 `SHIM_MCCOMPAT=0x800000001`을 `flyplay/__init__.py`에서 기본값으로 둡니다. 이 프로젝트의
  프로세스와 그 자식에만 적용됩니다(탐색기의 "그래픽 프로세서로 실행"이 쓰는 값이고, GZDoom도 컨텍스트 생성 전에 같은 방식으로
  설정합니다). 실행 파일이 내보내야 하는 `NvOptimusEnablement`는 파이썬에서 쓸 수 없습니다(GLFW 이슈 #980).

| 렌더링 | 겹눈 512×450 한 장 | 뷰어 1280×720 한 프레임(JPEG 포함) | 실험 워커 14개가 바쁠 때 파리 한 마리의 겹눈 |
|---|---:|---:|---:|
| Intel 내장 | 10.9~16.5 ms | 17.3 ms | 349~536 ms/시뮬레이션 초 |
| RTX 5060 | 0.9~1.9 ms | 8.5 ms | 85~95 ms/시뮬레이션 초 |

- 두 GPU는 색 경계를 조금 다르게 그립니다. 고정 시점 114개에서 낱눈 값 차이는 최대 0.022(4.5%의 낱눈이 조금이라도 다름)였고,
  눈 228개 중 17개(7.5%)에서 시각 케니언 세포 코드가 활성 5개 중 1개 달라졌습니다. 감각 점검은 둘 다 통과하지만 행동 지문은
  달라지므로, 이 날 21:48 이전에 시작한 실험은 Intel 렌더링 결과입니다.
- 배경 실험 풀(14프로세스)의 합산 속도는 다시 재지 않았습니다. 실험실의 예상 시간 상수(`POOL_SPEED_PER_FLY` 0.46)는 그대로라
  실제보다 조금 길게 나옵니다.
...
## 15. 샌드박스 초파리를 실제에 가깝게 — 데이터와 문헌 (2026-09-17)

사용자 요청: "개선할만한 점들 추천좀" → 추천 10개 가운데 "2번(기록에 코드 버전 남기기)은 안 해도 될 듯, 나머지 진행".
에이전트 네 개가 수용체 데이터, 버섯체 커넥톰, 움직임 시각, 항법·행동 수치를 조사했고, 아래는 본문이나 파일을 직접 열어
확인한 것만 적었습니다. 모든 추가 기능은 `SandboxConfig` 스위치이고, 모두 끄면 예전 초파리와 행동 지문이 똑같습니다(CLAUDE.md).

### 15.1 냄새 부호 — DoOR 수용체 반응 + hemibrain 배선 + APL

| 부분 | 쓴 것 | 결정과 이유 |
|---|---|---|
| 수용체 반응 | [DoOR 2.0.1](https://github.com/ropensci/DoOR.data) (Münch & Galizia 2016, *Sci Rep*; CC BY-SA 4.0). 693 물질 × 78 반응 단위, 값의 약 86%가 측정 없음 | 자발 발화(SFR)를 뺀 합의값, 억제는 0으로 자름, 측정 없음은 0. 성충 단위만. 사구체 51개 중 49개에 값이 있음(VA7m·VM6 없음) |
| 식초 | DoOR에는 순물질뿐 | 아세트산만으로는 식초 끌림에 필요한 DM1·VA2(Semmelhack & Wang 2009)가 오히려 약간 억제됨 → 아세트산 1 + 에틸아세테이트 0.5 + 아세토인 0.5 섞음(비율은 모델 선택) |
| 3-옥탄올 | DoOR의 Or13a 반응 0.58 | 시약에 섞인 1-옥텐-3-올 때문(Lüdke 등 2025) → 뺌 |
| 케니언 세포 배선 | [hemibrain v1.2](https://www.janelia.org/project-team/flyem/hemibrain) 전체 연결표(CC BY 4.0) + 사구체 이름([Schlegel 2021](https://github.com/flyconnectome/hemibrain_olf_data), MIT) | 케니언 세포 1927개(γ 36%, α′/β′ 17%, α/β 46%), 투사 뉴런 입력이 있는 세포 1761개. 시냅스 3개 이상을 발톱으로 셈 → 세포당 5.36개(광학현미경 5.2). 시각 종류를 빼면 냄새 케니언 세포 1733개(α/β 799, γ 599, α′/β′ 335) |
| 개체마다 다른 배선 | 발톱 배정은 개체마다 무작위에 가깝지만(Caron 2013) 세포 종류마다 선호 사구체가 있음(Zheng 2022; Hayashi 2022) | 세포마다 발톱 수와 시냅스 수는 그대로, 어느 사구체인지는 그 종류의 hemibrain 선호대로 시드마다 새로 뽑음. 종류별 선호의 코사인은 hemibrain과 0.97 이상 |
| APL 억제 | 냄새마다 케니언 세포 5–10%가 반응하고, APL을 막으면 부호가 촘촘해지고 비슷한 혼합물을 구별하지 못함(Lin 2014). APL은 국소적으로 억제(Amin 2020) | 고정 개수(top-k) 대신 되먹임 억제를 엽마다 따로. 냄새마다 활성 비율이 달라짐(2.4–11.8%, 중앙값 7.2%) |
| 구획 | 구획마다 지나가는 케니언 세포 엽이 다름(Aso 2014) | γ1pedc(빠른 처벌)=γ, α3(느린 처벌)=α/β, β′2+γ4(단맛)=α′/β′+γ, γ5+α1(영양)=γ+α/β |
| 시각 케니언 세포 | γd 99개, α/βp 60개(Li 2020) | 100개 → 159개, 엽 붙임 |

**결과.** 옥탄올과 MCH는 둘 다 Or69a(사구체 D)를 가장 세게 흥분시켜서 수용체 수준에서 이미 비슷합니다(코사인 0.47).
케니언 세포 부호는 0.33으로 덜 비슷해지지만(무작위 배선은 0.05), 설탕과 옥탄올을 짝지으면 MCH도 옥탄올 가치의 45%만큼 좋아집니다
(무작위 배선 10%). 방 안에서 설탕-옥탄올 훈련 뒤 선호 지수는 +0.90(무작위 +0.97, 4마리). 식초는 둘과 따로 부호화됩니다(0.05 미만).

### 15.2 움직임 시각 — T4/T5형 감지기, 시운동 반응, 그림자

| 항목 | 문헌 | 모델 |
|---|---|---|
| flyvis | 사전학습 모델 3.3 MB, MIT, Python <3.13, Linux에서만 시험. FlyGym 1.x 예제는 500 Hz로 돌리며 CPU에서 네트워크 한 번에 약 2초 | 실험 워커 14개가 CPU를 나눠 쓰는 여기서는 쓸 수 없음 → 가벼운 상관 감지기 |
| 감지기 | Borst 2018: 가운데 고역 통과 250 ms, 옆 저역 통과 50 ms, 10 ms 간격 | 그대로. ON(밝아짐)·OFF(어두워짐) 신호 각각에 Hassenstein-Reichardt 상관 |
| 격자 | FlyGym 낱눈 격자는 세로축 하나와 ±30° 축 둘 | 가로 움직임은 ±30° 축 둘을 합쳐 읽음 |
| 방향 조율 | T4/T5는 초당 1–3번 지나가는 무늬에 가장 셈(Creamer 2018), 걷는 파리의 회전은 약 8 Hz까지(Mano 2023) | 20° 주기 줄무늬에서 60–120°/s에 최대 |
| 시운동 반응 | 줄무늬를 따라 돎. 스스로 방향을 바꿀 때는 원심성 복사로 억제(Kim 2015). 걷는 파리의 회전 속도(°/s)는 본문에서 찾지 못함 | 두 눈의 넓은 시야 회전을 합쳐 tanh로 누르고 0.2초로 거른 만큼 돎(이득은 모델 선택, 15.6). 도약 회전 중에는 0 |
| 그림자 | 1초 간격으로 지나가는 그림자 횟수만큼 빨라지고 약 20초에 걸쳐 가라앉음(Gibson 2015). 다가오는 그림자에 천천히 걷던 파리 77%, 빨리 걷던 파리 27%가 얼어붙음(Zacarias 2018) | 눈 위쪽 시야가 전체로 어두워지면(OFF − ON이 0.15 넘음) 각성 +1(시간상수 20초). 조용한 5초 뒤의 첫 그림자만 속도에 따라 2초 얼어붙게 함 |
| 눈 표본 빈도 | 한 프레임에 낱눈 간격의 절반(약 2.5°) 넘게 움직이면 위신호 | 스스로 움직이는 것(도는 원통, 그림자)이 있을 때만 100 Hz, 없으면 10 Hz 그대로 |

### 15.3 중심복합체 — 나침반, 경로 적분, 장소 기억

| 부품 | 문헌 | 모델 |
|---|---|---|
| 나침반 | 16쐐기, 덩어리 폭 82°(Seelig & Jayaraman 2015). 방향 오차 확산 1.82×10⁻³ rad²/s, 1분 뒤 약 20–30°(Turner-Evans 2017 모델) | 자기 회전을 적분하고 같은 확산을 더함. 들어 올렸다 놓으면 방향 감각을 잃음 |
| 표지 | 고리 뉴런과 나침반 뉴런이 함께 활동하면 억제가 약해지고, 고리 뉴런만 활동하면 강해짐(Kim 2019). 재배열에 수 분(Fisher 2019). 어두운 막대 하나로도 덩어리가 고정됨(Seelig & Jayaraman 2015) | 위쪽 시야에서 어두운 것(밝기 0.3 미만)이 어느 방위에 얼마나 뚜렷하게 있는지. 뚜렷하면 그 방위를 나침반 좌표로 천천히(30초) 학습하고, 20초 넘게 본 뒤로는 지금 보이는 방위와 학습한 방위의 차이만큼 나침반을 당김. 낱눈 방위각은 원통의 줄무늬를 하나씩 켜 렌더링해 잼(왼눈 −12~+135°, 오른눈 −135~+14°) |
| 먼저 해 보고 버린 것 | — | 고리×쐐기 헤브 가중치: 당기는 덩어리로 다시 학습해 방향 추정이 몇 초 만에 100–200° 흔들리고 경로 적분이 100 mm 방에서 500 mm로 튐. 파노라마 전체를 상관으로 맞추기: 벽 근처에서는 벽이 시야를 채워 어떤 회전과도 상관이 높아, 내려놓은 뒤 6번 중 3번만 다시 맞춤. 벽의 회색까지 어둠으로 세면 가장 가까운 벽을 표지로 삼음 |
| 경로 적분 | CPU4 기억(Stone 2017) | 자기 움직임을 추정 방향으로 돌려 합산(벡터형) |
| 목표 | FC2가 목표 방향을 저장, PFL3가 오차의 사인으로 조향(Mussells Pires 2024; Westeinde 2024) | 뜨거운 바닥에서 시원한 곳에 닿는 순간 그 위치를 목표로 저장, 뜨거울 때 그쪽으로 조향. 주변 탐색도 참값 대신 추정값으로 돌아감 |
| 장소 기억 실험 | 36 °C 바닥의 시원한 칸 하나, 둘레의 막대 무늬, 5분 × 10회에 찾는 시간이 절반. 무늬를 돌리면 돌린 쪽을 찾음. 타원체 고리 뉴런이 필요하고 버섯체는 필요 없음(Ofstad 2011) | `heat_maze`, `heat_maze_plain`, `heat_probe` 방과 `place_memory` 세트 |

### 15.4 실제 초파리의 습성

| 습성 | 문헌 | 모델 |
|---|---|---|
| 벽 따라 걷기 | 8.4 cm 아레나에서 시간의 90–95%를 바깥 1/3에서 보냄. 앞을 못 보는 파리도 같음. 벽 6 mm 안에서 끌림(Soibam 2012) | 반사가 재는 옆 거리로 벽을 4.5 mm에 유지 |
| 개체 차이 | Y자 미로에서 23.5%가 한쪽으로 70% 넘게 돎, 반복성 0.81(Buchanan 2015). 여러 특성이 서로 거의 독립(Werkhoven 2021) | 좌우 편향 Beta(4.2, 4.2)(24.0%), 걷는 속도와 벽 선호의 개체 차. A/B 쌍둥이는 같은 개체 |
| 주변 탐색 | 먹이에서 경로 적분기를 0으로 맞추고 왕복 거리를 늘림(Behbahani 2021) | 되돌아서는 반경이 4 → 12 mm로 커짐 |

### 15.5 새 자극과 먹기

| 자극 | 문헌 | 모델 |
|---|---|---|
| 온도 | 24–25.5 °C 선호(Bang 2011; Kaneko 2012). 25/40 °C 경계에서 대부분 되돌아서고 더듬이 사이 0.1–0.2 °C 차로 방향이 정해짐(Simões 2021). 열 처벌은 전기의 도파민 뉴런 일부, γ1pedc 필요(Galili 2014; Otto 2020) | 더듬이가 몸보다 3 °C 따뜻하면 더 따뜻한 쪽 반대로 160° 돌기. 30 °C가 넘으면 처벌 도파민(34 °C에서 절반) |
| 쓴맛 | 발의 단맛 세포를 누름(Meunier 2003; Chu 2014). DEET 섞은 설탕과 짝지은 냄새는 곧바로 피함(Das 2014), 쓴맛 처벌에 PPL1 필요(Kirkhart & Scott 2015) | 단맛 × (1 − 쓴맛), 발에 닿는 동안 처벌 도파민 |
| 빛 도파민 | 도파민 뉴런에 CsChrimson을 넣고 빛을 비추면 전기·설탕 없이 기억이 생김(Aso & Rubin 2016; Claridge-Chang 2009) | 구역 안에서 처벌 또는 보상 도파민만 냄(배고픔 관문 없음) |
| 주둥이 | 순엽이 액체에 닿아야 펼쳐지고 빨아들임(Zhou 2019). 먹는 동안 0.2–2 mm/s로 조금씩 움직임(Corrales-Carvajal 2016) | 순엽이 방울 밖이면 접고 방울 가운데로 조금 옮긴 뒤 다시 뻗음 |

### 15.6 이번에 잰 것 — 문헌 값과 나란히

같은 시드의 켬/끔 쌍 4마리씩입니다. 조건과 전체 표는 PLAN.md F절.

| 행동 | 모델 | 실제 초파리 |
|---|---|---|
| 벽 따라 걷기 (빈 방 120초) | 바깥 1/3에 0.89 (끄면 0.72) | 0.90–0.95 (Soibam 2012) |
| 줄무늬 원통 (반시계 60°/s, 30초) | 4마리 모두 +17~+21°/s로 따라 돎 (멈춘 원통 −20~+19°/s, 벽을 따라 도는 방향대로) | 따라 돎. 걷는 파리의 °/s 값은 본문에서 못 찾음 |
| 그림자 (10초마다 1초 간격 5번, 60초) | 평균 16.1 mm/s (없으면 14.1), 2–8초 얼어붙음 | 지나간 횟수만큼 빨라짐 (Gibson 2015) |
| 쓴맛 0.6 섞은 식초 설탕 (60초) | 먹은 시간 9.0초 (없으면 12.8), 식초 가치 −0.22 (+0.75) | 덜 먹고, 짝지은 냄새를 곧바로 피함 (Das 2014) |
| 옥탄올 + 처벌 빛 3분 | 옥탄올 가치 −0.13~−0.23, MCH −0.02~−0.13 | 빛만으로 기억이 생김 (Aso & Rubin 2016) |
| 뜨거운 바닥의 시원한 칸 (45초 × 8번) | 마지막 두 시행 14.3초 (표지 없음 27.5초, 4쌍 모두). 그러나 처음 두 시행도 25.4초 대 34.1초 | 10번에 걸쳐 찾는 시간이 절반, 표지가 없으면 줄지 않음 (Ofstad 2011) |
| 커넥톰 뇌, 설탕과 옥탄올 | 시험 PI +0.90, MCH 가치 +0.50 (무작위 배선 +0.97, +0.04) | 옥탄올과 MCH는 수용체 반응이 겹침 (DoOR 코사인 0.47) |

나침반이 표지를 따르는 세기(`VISUAL_GAIN`)는 0.4와 1.0을 비교해 0.4로 정했습니다. 1.0에서도 내려놓을 때마다 방향
오차가 비슷하게 되돌아왔지만(시행 2–8의 합성 길이 0.72–0.96) 시원한 칸을 오히려 늦게 찾았습니다(39.2초). 이유는
따로 확인하지 않았습니다.

### 15.7 남은 한계

- 냄새는 벽을 돌아가는 거리의 역제곱이고, 공기 흐름·난류·퍼지는 시간은 없습니다(PLAN.md B-1의 확산 방정식 격자가 다음 단계).
- **장소 기억이 약합니다.** 표지가 있는 파리가 처음 두 시행부터 빨랐으므로, 마지막 시행의 차이가 모두 기억 덕분이라고
  말할 수 없습니다. 파리 4마리, 45초 시행으로 잰 것이고 `place_memory` 세트(60초 × 6번)는 시범 실행 전입니다.
- 중심복합체는 목표 하나(시원한 곳)만 기억하고, 먹이 장소나 전기 구역 자리는 목표가 되지 않습니다.
- 움직임 시각은 스스로 움직이는 자극이 있을 때만 계산합니다. 걸을 때 생기는 광류로 속도를 조절하는 반응 등은 없습니다.
  그림자 측정은 시운동 이득을 1.0으로 올리기 전(0.3)에 했습니다.
- 빛 학습은 옥탄올과 MCH의 가치 차이가 0.1 안팎이라, 가치 차이 0.2에서 시험 PI가 +0.02였던 전기 학습처럼 시험 PI가
  작게 나올 수 있습니다(예상, 재지 않음).
- 순엽이 방울에 닿아야 마시는 규칙은 켠 상태로만 돌려 봤습니다(쓴맛 대조군이 60초에 8.9–16.8초 먹음). 켬/끔 비교는 없습니다.
- 커넥톰은 한 개체(hemibrain)의 발톱 통계이고, MBON–DAN 되먹임과 MBON끼리의 연결은 기존 소거 신호 하나뿐입니다.
- 온도 반응은 경계에서 되돌아서기와 빠르게 걷기뿐이고, 몸 온도 변화는 계산하지 않습니다.
